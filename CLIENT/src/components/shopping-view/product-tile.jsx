...
function ShoppingProductTile({ product,handleGetProductDetails,handleAddToCart }) {
  return (
    <Card className="sm:max-w-[240px] w-full mx-auto overflow-hidden flex flex-col shadow-sm border  rounded-lg p-0">
      <div className="relative w-full h-[230px]" onClick={()=>handleGetProductDetails(product._id)}>
        <img
          src={product?.image}
          alt={product?.title}
      ...
      <CardContent className="p-2 flex-1 flex flex-col justify-between gap-1">
        <h2 className="text-sm font-semibold leading-tight truncate">{product?.title}</h2>
      ...
      <CardFooter className="px-2 pb-2">
        <Button onClick={()=>handleAddToCart(product?._id)} className="w-full h-8 text-xs">Add to Cart</Button>
      </CardFooter>
    </Card>
  );
}
